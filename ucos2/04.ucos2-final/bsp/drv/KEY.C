...
//    KeySelRow(KEY_ALL_ROWS);                     /* Select all row                                     */
    OSTaskCreate(KeyScanTask, (void *)0, &KeyScanTaskStk[KEY_SCAN_TASK_STK_SIZE], prio);
}
...
//    outp(KEY_PORT_CW, 0x82);                     /* Initialize 82C55: A=OUT, B=IN (COLS), C=OUT (ROWS) */
	...
	/* GPFCON -Input Mode GPF[3:4] */
	/* YOUR CODE HERE */
	rGPFCON &= ~(0xf<<6);
	rGPFUP |=(0x3<<3);
	
	/* GPBCON, GPHCON - outputmode GPB[0:2], GPH9, GPB4 */
	/* YOUR CODE HERE */
	rGPBDAT &= ~(0x1f);
	rGPBCON = (0x1<<8)+(0x15);
	rGPHDAT &= ~(0x1<<9);
	rGPHCON = rGPHCON & ~(0x3<<18)|(0x1<<18);
}
#endif
