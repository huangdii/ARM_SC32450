...
void Main(void)
{
	...
	// un-aligned access code insertion - start point
	char* p;
	...
	vl = *pl;
	Uart_Printf("After (%d) \n", vl);
  // un-aligned access code insertion - end point
	...
	Uart_Printf("%d \n\n", HOW_TO_RETURN(1,2,3,4,5,6,7)); //coded by assemblier
...
#if 1
{
	int rINTPND_1=0x80;  // no.7 bit = 1, // 0x8000;  // no.15 bit = 1
	Uart_Printf("interrupt number is %d\n",getIntNum(rINTPND_1));
}
#endif
