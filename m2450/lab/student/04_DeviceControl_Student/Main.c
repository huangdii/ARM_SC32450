 ...
 ***************************************/
	...
	//str = Uart_get_File(); //receiving file and show text
 ...
 ***************************************/
	...
	Timer_Init();
       ...
       HandleTIMER0 = (unsigned int)Timer0_ISR; //when timer is time-out, then this code is executed // register a ISR(interrupt service routine)
 ...
 ***************************************/
#if EXAMPLE == 426
	WDT_Init(5); // limitation (1sec ~ 32sec) // to reset after 3sec
	WDT_Start(1);
	for(;;) // endless loop
	{WDT_Clear2();} //endless loop
#endif
