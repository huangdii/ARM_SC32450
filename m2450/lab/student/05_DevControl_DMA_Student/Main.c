...
#if EXAMPLE == 420
// example of copy memory 
//** ISR Function declaration 
 __attribute__((interrupt("IRQ"))) void DMA0_ISR(void);
// Global Variables Declaration
// 목적지는 CACHE 영역이 아닐것 (DCache 'off')
unsigned int src=0x33F10000; //source memory register
unsigned int dst=0x33F20000;  // destination memory register
unsigned int size = 480; /* byte size */ //copy 480 byte 
unsigned int pattern;

 __attribute__((interrupt("IRQ"))) void DMA0_ISR(void)
{
	/* TODO: Pendng Clear on DMA0 */
	rSUBSRCPND = 0x1<<18; // interrupt state clear 
	rSRCPND1 = 0x1<<17;
	rINTPND1 = 0x1<<17;
...
{	
	...
	Uart_Init(115200);	
	DMA0_SW_Init(); // (1)DMA Initialization 
	...
	HandleDMA = (unsigned int)DMA0_ISR; // (2)when DMA Executes, handler is executed. // channel 0 DMA

	/*  인터럽트 허용 on DMA0 */		
	rINTMSK1 &= ~(0x1<<17);  // (3)Release mask 

	DMA0_SW_Start(); // (4)DMA start
...
while(1)
	{
		...
		if(dmaComplete)
			{
			elapseTime = ElapseTimer_Stop();
			print("elapseTime=%d(msec)",elapseTime); // To count elapsed time so far // elapseTime/frequency ... you should add..
			print("cnt = %d\n",cnt); //To check simultaneously executed CPU using cnt value when mem 2 mem  
			}
	}
