...
void DMA0_SW_Init(void)
{
	...
	rDISRC0 = src;//source memory register
	rDIDST0 = dst;// destination memory register
	...
	| (1 << 22) | (2 << 20) | ((size/4) << 0); //(size/4)=480/4=120
...
{
	/* Init DMASKTRIG on DMA0*/
	rDMASKTRIG1 = 0x0;  //Register related to DMA Start
	
	/* TODO : Select Base Address of Source and Destination*/
	rDISRC1 = &rURXH1; //rURXH1 register's address //source--> uart's Rx 
	rDIDST1 = dst;
	
	/* TODO : Init Source & Destination Control Register on DMA1 
	 * (memory -> AHB bus, INC mode, CHK_INT -> after auto-reload )
   	 */
	// SOURCE ATTRIBUTE/ APB, no Increment, 1 byte unit 
	...
	rDMAREQSEL1 = (rDMAREQSEL1 &~ (0x1f<<1))|(22<<1); // hardware trigger   22 -> Rx
	rDMAREQSEL1 |= 1;
...
void DMA0_SW_Start(void)
{
	...
	rDMASKTRIG0 |= 1; // DMA START!!!  // mem 2 mem only 
}
